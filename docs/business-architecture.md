...
核心能力：3-Tick 跳动预警，捕捉短时间内的价格异动。

#### 检测机制

每次 `FastDataPoller` 获取到新价格后，调用 `check_tick_jump(market, price, source)` 进行检测：

1. **Tick 环形缓冲**：每个品种维护一个最大容量为 5 的环形队列（`*_tick_ring`），每次追加最新 tick 并淘汰最老的。
2. **3-Tick 窗口比较**：取倒数第 3 个 tick 与最新 tick，计算百分比变化 `change_pct = (last - first) / first × 100`。
3. **阈值判定**：`|change_pct| ≥ 该品种阈值` 时触发告警。

#### 分品种阈值

阈值按品种独立配置，与动量策略的 `spread_entry` 参数对齐，确保 tick 预警灵敏度与信号捕捉一致：

| 品种 | 市场键 | 默认阈值 | 动量 spread_entry | 设计依据 |
|------|--------|---------|-------------------|----------|
| 沪银 | hu | 0.15% | 0.15% | 对齐 spread_entry |
| COMEX 银 | comex | 0.10% | 0.05% | 2× spread_entry，COMEX 波动更快 |
| 沪金 | hujin | 0.12% | 0.12% | 对齐 spread_entry |
| COMEX 金 | comex_gold | 0.10% | 0.05% | 2× spread_entry |
| BTC | btc | 0.30% | 0.08% | ~4× spread_entry，高波动品种适当放宽 |

阈值优先级：`state.tick_jump_thresholds[market]` → `state.tick_jump_threshold`（全局回退值）。

#### 严重等级

严重等级按**阈值倍数**判定，而非绝对百分比，使不同品种的分级标准统一：

| 等级 | 条件 | 含义 |
|------|------|------|
| LOW | 1× ≤ ratio < 2× | 刚触发阈值，短期异动 |
| MEDIUM | 2× ≤ ratio < 3× | 显著异动，值得关注 |
| HIGH | ratio ≥ 3× | 极端行情，需立即关注 |

其中 `ratio = |change_pct| / threshold`。

#### 告警输出

触发后生成告警对象，包含：方向（急涨/急跌）、变化幅度、起止价格与时间、数据源、严重等级。告警写入 `state.alert_history`（最多 200 条）和 `state.alert_stats` 统计计数。

前端通过 `GET /api/alerts` 轮询获取告警列表和 tick 环数据，新告警自动触发横幅提示和声音播报（急涨 880Hz、急跌 440Hz）。

#### 阈值调整

- 配置文件：`monitor.config.json` → `alerts.tick_jump_thresholds`
- 运行时 API：`POST /api/threshold`，支持全局或分品种调整
- 前端下拉：0.05%–1.00%，步进 0.05%
